...
pub const MAINTENANCE: &[CommandSpec] = &[
    ...
    CommandSpec {
        name: "upgrade",
        aliases: &[],
        summary: "Replace the installed binary with a verified artifact",
        usage: "rune upgrade --from <path> --checksum <hex> [--target <path>] [--json]",
        flags: &[
            option("--from", "path", "Artifact to install."),
            option("--checksum", "hex", "Published checksum of that artifact."),
            option(
                "--target",
                "path",
                "Binary to replace. Defaults to the running one.",
            ),
            flag("--json", "Emit JSON."),
        ],
        requirements: Requirements::NONE,
    ...
    CommandSpec {
        name: "uninstall",
        aliases: &[],
        summary: "Remove the installed binary and, on request, local state",
        usage: "rune uninstall [--target <path>] [--keep-state] [--yes] [--json]",
        flags: &[
            option(
                "--target",
                "path",
                "Binary to remove. Defaults to the running one.",
            ),
            flag("--keep-state", "Leave the state directory in place."),
            flag("--yes", "Confirm removal of the state directory."),
            flag("--json", "Emit JSON."),
        ],
        requirements: Requirements::CONFIG,
        supports_json: true,
