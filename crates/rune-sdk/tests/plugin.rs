...
/// A scripted plugin, written to a temporary directory.
struct Fixture {
    directory: tempfile::TempDir,
}

impl Fixture {
    /// Writes a script and a manifest that starts it.
    ///
    /// `script` is a complete shell program; the manifest names it, and the
    /// tools it declares are the standard pair.
    fn new(name: &str, script: &str, tools: &str) -> Self {
        let directory = tempfile::tempdir().expect("temp dir");
        let program = directory.path().join("plugin.sh");
        std::fs::write(&program, script).expect("write script");
        let manifest = json!({
            "protocol_version": 1,
            "name": name,
            "tools": serde_json::from_str::<Value>(tools).expect("tool list"),
            "commands": ["/bin/sh", program.to_str().expect("path")],
        });
        std::fs::write(
...
/// A shell prologue that answers the handshake.
///
/// Every script reads a line at a time and extracts the request identifier, so
/// a fixture only has to supply the dispatch for anything past `initialize`.
...
/// A dispatch that answers both tools, and records every frame it receives to
/// its own stdout, prefixed so the test can tell a record from a response.
const ECHOING_DISPATCH: &str = r#"
    *'"method":"tools/call"'*)
      printf '{"received":%s}\n' "$line"
...
/// A fixture that records every request it receives to a file beside itself, so
/// a test can read exactly what the plugin was sent.
#[test]
fn a_denied_call_leaves_no_trace_in_the_plugin() {
    let directory = tempfile::tempdir().expect("temp dir");
    let journal = directory.path().join("journal.txt");
    let program = directory.path().join("plugin.sh");
...
"#,
    ...
    let recorded = std::fs::read_to_string(&journal).expect("journal");
    assert!(
...
fn a_crash_looping_plugin_is_disabled_after_its_restart_limit() {
    ...
    let fixture = Fixture::new("crasher", "#!/bin/sh\nexit 3\n", TOOLS);
...
fn a_plugin_that_never_answers_the_handshake_times_out() {
    // Reads its input but never writes anything.
...
fn an_incompatible_handshake_version_is_refused_naming_both_versions() {
    ...
    std::fs::write(
        &program,
        "#!/bin/sh\nwhile IFS= read -r line; do\n  id=$(printf '%s' \"$line\" | sed -n 's/.*\"id\":\\([0-9][0-9]*\\).*/\\1/p')\n  printf '{\"jsonrpc\":\"2.0\",\"id\":%s,\"result\":{\"protocol_version\":9}}\\n' \"$id\"\ndone\n",
    )
    .expect("write script");
    std::fs::write(
        directory.path().join("plugin.json"),
    ...
    let err = PluginHost::start_with_limits(directory.path(), limits(2_000, 0))
...
fn a_frame_larger_than_the_cap_is_refused() {
    let directory = tempfile::tempdir().expect("temp dir");
    let program = directory.path().join("plugin.sh");
    // Answers the handshake, then floods one frame past the cap.
    std::fs::write(
        &program,
        format!(
            "#!/bin/sh\nwhile IFS= read -r line; do\n  id=$(printf '%s' \"$line\" | sed -n 's/.*\"id\":\\([0-9][0-9]*\\).*/\\1/p')\n  case \"$line\" in\n    *'\"method\":\"initialize\"'*)\n      printf '{{\"jsonrpc\":\"2.0\",\"id\":%s,\"result\":{{\"protocol_version\":1}}}}\\n' \"$id\"\n      ;;\n    *)\n      printf '{{\"jsonrpc\":\"2.0\",\"id\":%s,\"result\":\"' \"$id\"\n      dd if=/dev/zero bs=1024 count={} 2>/dev/null | tr '\\0' 'a'\n      printf '\"}}\\n'\n      ;;\n  esac\ndone\n",
            (MAX_FRAME_BYTES / 1024) + 16
        ),
    )
    .expect("write script");
    std::fs::write(
    ...
    let mut host =
        PluginHost::start_with_limits(directory.path(), limits(5_000, 0)).expect("start");
    let err = host
        .call("echo", &json!({}))
...
"#,
    ...
    let mut host =
        PluginHost::start_with_limits(directory.path(), limits(2_000, 1)).expect("start");
